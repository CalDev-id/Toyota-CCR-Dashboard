...
const parts = new Set(["cylblock", "cylhead", "camshaft", "crankshaft"]);
...
function parseRatio(value: unknown) { const [one, two] = String(value ?? "1:1").split(":").map(Number); return [Math.max(0, one || 0), Math.max(0, two || 0)] as const; }
...
async function getPlanContext(part: string, date: string, shift: string, group: string) {
  ...
  const source = await db.$queryRawUnsafe<Record<string, unknown>[]>(`SELECT ftt, foee, fratio, fot FROM \`${table}\` WHERE fdate=? AND fshift=? AND fgroup=? LIMIT 1`, date, shift, group);
  const monthlyTt = Number(source[0]?.ftt ?? 0); const monthlyRatio = String(source[0]?.fratio ?? "1:1"); const monthlyOee = Number(source[0]?.foee ?? (part === "camshaft" ? 93 : 90)); const otMinutes = Math.max(0, Math.round(Number(source[0]?.fot ?? 0) * 60));
  await db.$executeRawUnsafe("INSERT IGNORE INTO t_daily_production_plan (line_key,fdate,fshift,fgroup) VALUES (?,?,?,?)", part,date,shift,group);
  const plans = await db.$queryRawUnsafe<Plan[]>("SELECT id,override_tt,override_ratio,source_tt,source_oee,source_ratio,source_ot_minutes FROM t_daily_production_plan WHERE line_key=? AND fdate=? AND fshift=? AND fgroup=? LIMIT 1", part,date,shift,group);
  const plan = plans[0]; if (!plan) throw new Error("Unable to create daily plan");
  const monthlyChanged = plan.source_tt === null || Number(plan.source_tt) !== monthlyTt || Number(plan.source_oee) !== monthlyOee || plan.source_ratio !== monthlyRatio || Number(plan.source_ot_minutes) !== otMinutes;
  if (monthlyChanged) {
  ...
  const tt = Number(plan.override_tt ?? monthlyTt); const ratio = plan.override_ratio || monthlyRatio;
  return { db, plan, tt, ratio, monthlyOee, otMinutes };
}
...
export async function loadDailyPlanning(part: string, date: string, shift: string, group: string) {
  ...
  const rows = slots.map((slot) => { const oee = slot.is_oee_override ? Number(slot.oee) : monthlyOee; const target = slot.is_schedule_override ? Number(slot.total_target) : targetFor(Number(slot.prod_minutes),tt,oee); const [oneTr,twoTr] = slot.is_schedule_override ? [Number(slot.one_tr),Number(slot.two_tr)] : split(target,ratioOne,ratioTwo); return { ...slot, oee: Number(slot.oee ?? 0), ftt:tt, foee:oee, fratio:ratio, ftotal_target:target, f1tr:oneTr, f2tr:twoTr }; });
  return { group, tt, oee: monthlyOee, ratio, ratioOne, ratioTwo, rows };
}
...
export async function updateDailySharedParameters(part: string,date: string,shift: string,group: string,tt: number,ratio: string) { await requireUser(); const [one,two] = parseRatio(ratio); if (tt<=0 || one+two<=0) throw new Error("TT dan Ratio harus valid"); const { db,plan } = await getPlanContext(part,date,shift,group); await db.$executeRawUnsafe("UPDATE t_daily_production_plan SET override_tt=?,override_ratio=? WHERE id=?",tt,`${one}:${two}`,plan.id); revalidatePath("/daily-planning"); }
export async function updateDailySlotSchedule(id: number,startTime: string,endTime: string,minutes: number,ratioOne: number,ratioTwo: number,tt: number,oee: number) { await requireUser(); const target=targetFor(Math.max(0,Math.round(minutes)),tt,oee*100); const [oneTr,twoTr]=split(target,ratioOne,ratioTwo); await getReportPrisma().$executeRawUnsafe("UPDATE t_daily_production_plan_slot SET start_time=?,end_time=?,prod_minutes=?,total_target=?,one_tr=?,two_tr=?,is_schedule_override=1 WHERE id=?",startTime,endTime,Math.max(0,Math.round(minutes)),target,oneTr,twoTr,id); revalidatePath("/daily-planning"); }
