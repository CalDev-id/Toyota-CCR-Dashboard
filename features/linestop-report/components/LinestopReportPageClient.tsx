...
type Summary = { key: LineKey; label: string; taktTime: number | null; unmappedMinutes: number; rows: Array<{ machineName: string; minutes: number }>; unmappedRows: Array<{ machineName: string; minutes: number }> };
...
function getChartScale(maximum: number) {
  if (maximum <= 0) return { maximum: 1, ticks: [1, 0] };
  const roughStep = (maximum * 1.1) / 5;
  const magnitude = 10 ** Math.floor(Math.log10(roughStep));
  const normalized = roughStep / magnitude;
  const multiplier = normalized <= 1 ? 1 : normalized <= 2 ? 2 : normalized <= 5 ? 5 : 10;
  const step = multiplier * magnitude;
  const axisMaximum = Math.max(step, Math.ceil((maximum * 1.1) / step) * step);
